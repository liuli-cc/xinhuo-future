...
export default function InterviewPage() {
  ...
  const startInterview = () => {
    ...
    const openingQuestion = questions[0]?.label ?? "你好，我是今天的面试官cbb老师。先不用紧张，请你结合正在申请的方向，做一个两分钟左右的自我介绍。";
  ...
  if (pageMode === "setup") {
    return (
      ...
      <PortalFrame active="interview" eyebrow="INTERVIEW STUDIO" title="和 cbb 老师，完成一场真实的模拟面试" subtitle="多格式简历识别 · 连续语音追问 · 约 15 分钟 · 结构化复盘">
        <div className="voice-interview-setup interview-v3">
          ...
          <aside className="setup-steps-panel">
            ...
            <div className={`step-item ${setupStep === "ready" ? "active" : ""}`}>
              ...
              <div><b>开始实时通话</b><small>cbb老师自然追问，Chrome 自动转写回答</small></div>
            </div>
          </aside>
            ...
            <div className="live-transcript-feed" aria-live="polite">
              {conversationLog.length === 0 && (
                <div className="live-call-empty">
                  <span>通话即将开始</span>
                  <p>cbb老师会先介绍本次面试，然后自然地引导你完成后续交流。</p>
                </div>
              )}
              {conversationLog.slice(-7).map((entry, logIndex) => (
                <article className={`live-transcript-entry ${entry.speaker}`} data-live-entry key={`${entry.speaker}-${logIndex}-${entry.text.slice(0, 12)}`}>
                  <span>{entry.speaker === "interviewer" ? "cbb老师" : "我"}</span>
                  <p>{entry.text}</p>
                </article>
              ))}
            ...
            <div className="live-turn-console" data-live-console>
              ...
              {callPaused && (
                <div className="live-paused-note">
                  <b>通话已暂停</b>
                  <span>点击页面右上角“继续通话”，cbb老师会从当前问题继续。</span>
                </div>
              )}
